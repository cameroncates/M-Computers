<template>
<div class="container-fluid pt-5 pb-5 bg-white bd-bottom">
    <div class="m-auto p-2 prebuilds-search-container">
        ...
        <div class="w-100 mb-2" v-if="filter.selected.length > 0">
            <button @click="removefrom_filter(item)" class="btn bg-light-2 pl-4 pr-4 text-sm hov-danger mr-2 mb-2 bd-round animate__animated  animate__fadeIn animate__faster" v-for="(item, i) in filter.selected" :key="i">{{item}}</button>
            <br>
        </div>
        <div class="input-group bg-light box-shadow p-1">
            ...
            <div class="input-group-append">
            <button class="btn input-group-text btn-hov bg-none bd-none bd-radius-5" @click="visible = !visible" :class="visible ? 'text-primary bg-light-2' : ''">
        ...
        <div class="table-responsive bg-light p-2 box-shadow animate__animated  animate__fadeIn animate__faster " v-if="visible">
            <table class="table table-borderless">
                <thead>
                <tr class="bd-bottom">
                    <th class="smallfont-weight-bold" v-for="(item, i) in filter.title" :key="i">{{item}}</th>
                </tr>
                </thead>
                <tbody>
                    <tr v-for="(item, j) in filter.list" :key="j">
                        <td style="min-width:150px" class="pb-1 pt-1" v-for="(sub_item, k) in filter.list[j]" :key="k">
                            <button @click="addto_filter(sub_item)" class="btn text-sm btn-block p-1 btn-hov  text-left" :class="filter.selected.includes(sub_item) ? 'text-primary font-weight-bold': ''">{{sub_item}}</button>
                        </td>
                    </tr>
                </tbody>
            </table>
